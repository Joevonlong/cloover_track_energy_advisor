...
- `apps/frontend/` — Vite + React + TS + Tailwind SPA (**pnpm**, registered in `pnpm-workspace.yaml`). TanStack Query, React Hook Form + Zod.
...
- **No secrets in the frontend bundle.** The only frontend env var is `VITE_API_BASE_URL`; every API key (Anthropic, Tavily, Supabase service-role, Google) lives in the backend's env via `pydantic-settings` (`apps/backend/src/app/core/config.py`).
...
The frontend screens live under `apps/frontend/src/features/`: `intake/` (RHF + Zod form), `dashboard/` (hero number + charts), `results/` (scenario cards), `configurator/` (4-rung ladder toggles), `proposal/` (CTA + LLM copy).
