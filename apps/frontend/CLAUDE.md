...
- `features/intake/` — `IntakeForm` (React Hook Form + Zod via the custom `zodResolver`; the street
  field is a Mapbox geocode autocomplete) and the `IntakeScreen` orchestrator. `householdSchema.ts`
  is the Zod schema for the `Household` contract type.
...
the feed + a status pill with the resulting €/month figure.
...
not reachable from `App.tsx` today — the flow currently ends at the 3D viewer + feed. Wire these in
when implementing the post-recommendation screens (F20–F23).
