...
R-D (§6.6, FEATURE_BACKLOG §2.1): render `Recommendation.strategies[]` (three `Strategy` objects emitted by F27) as three comparable, Check24-style option cards — replacing or supplementing the four-rung `alternatives[]` scenario cards — so the user can compare payback profiles at a glance before committing.

- **Three strategy cards** — render one card per strategy in the order `strategies[]` delivers them: **Optimal** (default, highlighted CTA — `id: "optimal"`), **Fastest payback** (`id: "fastest"`), **Long-term** (`id: "longterm"`). Each card reads its `Strategy.scenario: ScenarioResult` for the financial figures and `Strategy.rationale` for the one-line explanation. Source: `Recommendation.strategies[]` (F02 contract extension from F27).
- **Per-card financials** — each card must show: monthly saving (`scenario.monthly_saving_eur`), capex after subsidy (`scenario.capex_after_subsidy_eur`), monthly installment (`scenario.installment_eur_month`), and break-even (`Strategy.break_even_month` or `scenario.break_even_month`). This satisfies R-E for strategy-level comparisons (§9) and lets the user weigh financing risk vs return at a glance.
- **One-line rationale** — render `Strategy.rationale` (a short prose string from F27/F16) beneath each card's financial summary: e.g. "Best ROI — reaches cumulative profit soonest; lowest financing risk" for the Fastest card (§6.6 label examples).
- **"Optimal" as the default CTA** — the Optimal card carries the primary green CTA ("Apply for Cloover financing"); the other two cards show a secondary "Select this plan" action. Clicking any card locks that `ScenarioResult` as the selected rung, updating the hero (F21), buckets, and the configurator (F20) in sync (existing AC5 applies).
- **Partial-config cost + break-even** — the four existing `alternatives[]` cards (solar-only through full bundle) must also show `capex_after_subsidy_eur` + `installment_eur_month` + `break_even_month` inline, consistent with R-E (§9); the solar-only card shows the break-even with the up-sell hook rather than a standalone profit date.
- **AC addendum** — add AC8: given the §8 `Recommendation.strategies[]`, when the three strategy cards render, then Optimal is highlighted with the primary CTA, each card shows saving + capex + installment + break-even + rationale, and selecting a non-default card updates F20/F21 correctly.
