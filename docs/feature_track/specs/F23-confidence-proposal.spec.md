...
R-F (§4, FEATURE_BACKLOG §2.1): the permits panel already exists (R6/AC6); this block clarifies that it is the product's **visible permit layer** and specifies the exact flags to render from `SiteCheckResponse.feasibility_flags[]`.

- **Permits panel is the permit layer** — per §4 R-F clarification, this panel is the definitive display of all planning/legal obligations for the configured products. It must always be visible in the dashboard (not collapsed by default), positioned near the assumptions drawer so the user sees trust signals together.
- **Flag set to render** — iterate `SiteCheckResponse.feasibility_flags[]` (`FeasibilityFlag{product, check, status, message}`) and render one row per flag using the canonical status chips: 🟢 (green — no action needed), 🟡 (amber — conditional approval), ℹ️ (info — registration obligation). The mandatory flag checks, per §4, are: roof/PV verfahrensfrei, Denkmalschutz gate, GEG §71 compliance, WEG/§554 BGB EV right, grid registration (≤11 kW notify / >11 kW approve), and battery MaStR registration. Render each that the Site-Check response includes; omit any product whose layer is not selected.
- **Assumptions drawer + cited subsidies** — the assumptions drawer (R2–R4) must keep `source_url` visible per subsidy assumption entry. Each `Assumption` that corresponds to a subsidy row carries the `source_url` from `subsidy_catalog` (F26/§12.1); render it as a linked citation ("Source: KfW 458", linking to the official page). This lets the user verify the subsidy claim at a glance without leaving the dashboard.
- **AC6 extension** — extend the existing AC6 assertion: given a `SiteCheckResponse` with a Denkmalschutz 🟡 flag and a grid-registration ℹ️ flag, the permits panel must show both alongside the green flags, using the correct chip colour and the `message` text; the "Free with the bundle" list remains visible beneath the panel.
- **No new contract fields** — `FeasibilityFlag` and `SiteCheckResponse` are already in F02; the F26 `subsidy_catalog` `source_url` reaches the FE via `Recommendation.assumptions[]` entries (the `source` field of `Assumption`); no additional endpoint or schema change is required for F23.
