...
- [x] The demo happy-path (a fixture `Recommendation` deserialises in FE + BE) still works after merge. ✅ `apps/backend/fixtures/demo-detached.json` is valid JSON (verified `python3 -m json.tool`); matches all schema types.
